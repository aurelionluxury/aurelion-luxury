    ...
    <div style={{ background: "#0a0a0c", minHeight: "100vh", paddingTop: 140, paddingBottom: 100 }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 clamp(24px,5vw,80px)" }}>
        ...
        <form action="/search" method="GET" style={{ marginBottom: 60 }}>
          <div style={{ display: "flex", gap: 0, maxWidth: 600, border: "1px solid rgba(212,175,55,0.3)", background: "rgba(255,255,255,0.02)" }}>
            <input
              name="q"
              defaultValue={q}
              placeholder="Search properties, cars, articles..."
              style={{ flex: 1, background: "transparent", border: "none", outline: "none", padding: "14px 20px", fontSize: 14, color: "rgba(255,255,255,0.8)", fontFamily: "'DM Sans', sans-serif" }}
            />
            <button type="submit" style={{ background: gold, border: "none", padding: "14px 24px", cursor: "pointer", color: "#0a0a0c", fontSize: 13, letterSpacing: "0.08em", textTransform: "uppercase", fontWeight: 500 }}>
              Search
            </button>
          </div>
        </form>
        ...
        {properties.length > 0 && (
          <Section title="Properties">
            {properties.map((p) => (
              <ResultCard key={p.id} href={`/real-estate/${p.slug}`} title={p.title} subtitle={p.location} tag={p.type} meta={p.priceLabel ?? undefined} />
            ))}
          </Section>
        )}
        ...
        {vehicles.length > 0 && (
          <Section title="Vehicles">
            {vehicles.map((v) => (
              <ResultCard key={v.id} href={`/cars/${v.slug}`} title={v.title} subtitle={`${v.make} · ${v.year}`} tag="Automobile" meta={v.priceLabel ?? undefined} />
            ))}
          </Section>
        )}
        ...
        {posts.length > 0 && (
          <Section title="Blog & Insights">
            {posts.map((p) => (
              <ResultCard key={p.id} href={`/blog/${p.slug}`} title={p.title} subtitle={p.excerpt ?? ""} tag={p.category} />
            ))}
          </Section>
        )}
        ...
        {faqs.length > 0 && (
          <Section title="FAQs">
            {faqs.map((f) => (
              <ResultCard key={f.id} href="/faq" title={f.question} subtitle={f.answer} tag={f.category} />
            ))}
          </Section>
        )}
      </div>
