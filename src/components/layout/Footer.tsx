...
export default async function Footer() {
  const settingsRaw = await prisma.siteSetting.findMany({
    where: { key: { in: ["instagram_url", "instagram", "linkedin_url", "linkedin", "facebook_url"] } }
  }).catch(() => []);

  const s: Record<string, string> = {};
  settingsRaw.forEach(r => { s[r.key] = r.value; });

  const instagramUrl = s.instagram_url || s.instagram || "";
  const linkedinUrl = s.linkedin_url || s.linkedin || "";
  const facebookUrl = s.facebook_url || "";
